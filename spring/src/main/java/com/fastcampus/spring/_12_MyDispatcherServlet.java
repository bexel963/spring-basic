...
// _12_MethodCall3의 원리를 이용해 원격으로 Controller 호출하는 프로그램

// @WebServlet은 @Controller + @RequestMapping 두개 합친거 - Servlet은 메서드 단위로 매핑이 안되고 클래스 단위로만 매핑할 수 있다.
// Servlet은 HttpServlet을 상속을 받아야한다.
// 메서드는 꼭 service라는 이름으로 해야하고 매개변수도 request, response 두개를 꼭 넣어줘야한다.
...
public class _12_MyDispatcherServlet extends HttpServlet {	
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// URL요청 - http://localhost:8080/spring/myDispatcherServlet?year=2022&month=3&day=13 
		Map    map = request.getParameterMap();	// year=2022&month=3&day=13 을 가져와 맵으로 저장한다.
		Model  model = null;
		String viewName = "";
		...
		try {
			...
			for(int i=0;i<paramArr.length;i++) {
				...
				if(paramType==Model.class) {
					...
					String strValue = ((String[])value)[0];	// getParameterMap()에서 꺼낸 value는 String배열이므로 변환 필요 (map에서 key는 string인데 value만 string[] 임)
